...
from ttpo_voting import extract_boxed_answer
...
class StepTimerCallback(TrainerCallback):
    """Record wall-clock seconds elapsed per global optimization step."""
...
class OPSDTrainer(SFTTrainer):
    ...
    def __init__(
        ...
        use_greedy_privileged: bool = False,
        greedy_max_tokens: int = 16000,
    ):
        ...
        self.use_greedy_privileged = use_greedy_privileged
        self.greedy_max_tokens = greedy_max_tokens
        ...
        if data_collator is None:
            data_collator = SelfDistillationDataCollator(
                ...
                use_greedy_privileged=use_greedy_privileged,
            )
        ...
        if self.use_greedy_privileged:
            if self.reason_first:
                raise ValueError(
                    "use_greedy_privileged=True and reason_first=True are mutually exclusive. "
                    "Greedy privileged builds teacher prompts dynamically from greedy answers."
                )
            if not getattr(args, "use_vllm", False):
                raise ValueError(
                    "use_greedy_privileged=True requires use_vllm=True for efficient greedy generation."
                )
            print(f"\n{'='*80}")
            print("GREEDY PRIVILEGED MODE ENABLED")
            print(f"Teacher sees model's own greedy answer (max {greedy_max_tokens} tokens) as privileged info.")
            print("No ground-truth solution is used for teacher prompts.")
            print(f"{'='*80}\n")
    ...
    def _set_signature_columns_if_needed(self):
        ...
        if self._signature_columns is None:
            ...
            # Keep "solution" and "answer" if present in dataset
            for optional_col in ["solution", "answer"]:
                if optional_col not in self._signature_columns:
                    self._signature_columns.append(optional_col)

    def _prepare_dataset(self, dataset, *args, **kwargs):
        # Skip SFTTrainer's default tokenization — our custom data collator handles everything.
        return dataset
    ...
    def _dist_reduce(self, values, op=dist.ReduceOp.SUM):
        """All-reduce a list of scalars across processes and return the reduced python list.

        This is a collective op: it must be called unconditionally on every rank, never behind
        a rank-divergent branch.
        """
    ...
    @staticmethod
    def _entropy_sum_and_count(logits, mask, temperature):
        """Token-weighted entropy sum over masked positions, accumulated one row at a time.

        Row-wise iteration keeps the fp32 upcast at [T, V] instead of [B, T, V], which matters
        for the long completions OPSD trains on.
        """
        total = 0.0
        count = 0.0
        with torch.no_grad():
            for i in range(logits.shape[0]):
                ...
                if not row_mask.any():
                    continue
                log_probs = F.log_softmax(logits[i].float() / temperature, dim=-1)
                entropy = -(log_probs.exp() * log_probs).sum(dim=-1)
                total += entropy[row_mask].sum().item()
                count += row_mask.sum().item()
        return total, count
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        student_ent_sum, ent_token_count = self._entropy_sum_and_count(
            student_logits.detach(), entropy_mask, self.temperature
        )
        ...
        with torch.no_grad(), adapter_context:
            ...
            teacher_ent_sum, _ = self._entropy_sum_and_count(
                teacher_logits, entropy_mask, self.temperature
            )
        ...
        g_student_sum, g_teacher_sum, g_token_count = self._dist_reduce(
            [student_ent_sum, teacher_ent_sum, ent_token_count]
        )
        if g_token_count > 0:
            self._metrics["train"]["student_entropy"].append(g_student_sum / g_token_count)
            self._metrics["train"]["teacher_entropy"].append(g_teacher_sum / g_token_count)
            self._metrics["train"]["entropy_gap"].append(
                (g_teacher_sum - g_student_sum) / g_token_count
            )
    ...
    def _generate_greedy_privileged_vllm(self, prompts_text: list[str]) -> tuple[list[list[int]], list[str]]:
        """Generate greedy (temperature=0) completions for privileged information."""
        import time

        sampling_params = SamplingParams(
            n=1,
            temperature=0.0,
            max_tokens=self.greedy_max_tokens,
        )
        ...
        if self.vllm_mode == "colocate":
            if hasattr(self, "vllm_tp_group") and self.vllm_tensor_parallel_size > 1:
                ...
                gathered_prompts = [None for _ in range(self.vllm_tensor_parallel_size)]
                torch.distributed.all_gather_object(
                    gathered_prompts, prompts_text, group=self.vllm_tp_group
                )
                all_prompts_text = [p for sublist in gathered_prompts for p in sublist]
            else:
                all_prompts_text = prompts_text
            ...
            completion_token_ids = [list(out.outputs[0].token_ids) for out in all_outputs]
            completion_texts = [out.outputs[0].text for out in all_outputs]

            if hasattr(self, "vllm_tp_group") and self.vllm_tensor_parallel_size > 1:
                local_rank_in_group = torch.distributed.get_rank(group=self.vllm_tp_group)
                tp_slice = slice(local_rank_in_group * orig_size, (local_rank_in_group + 1) * orig_size)
                completion_token_ids = completion_token_ids[tp_slice]
                completion_texts = completion_texts[tp_slice]

        elif self.vllm_mode == "server":
            all_prompts_text = gather_object(prompts_text)
            if self.accelerator.is_main_process:
                raw_ids = self.vllm_client.generate(
                    prompts=all_prompts_text,
                    n=1,
                    temperature=0.0,
                    max_tokens=self.greedy_max_tokens,
                ...
                raw_ids = [None] * len(all_prompts_text)
            raw_ids = broadcast_object_list(raw_ids, from_process=0)
            process_slice = slice(
                self.accelerator.process_index * len(prompts_text),
                (self.accelerator.process_index + 1) * len(prompts_text),
            )
            completion_token_ids = [list(ids) for ids in raw_ids[process_slice]]
            completion_texts = [
                self.processing_class.decode(ids, skip_special_tokens=False)
                for ids in completion_token_ids
            ]
        else:
            raise ValueError(f"Unknown vllm_mode: {self.vllm_mode}")

        elapsed_time = time.time() - start_time
        total_tokens = sum(len(ids) for ids in completion_token_ids)
        print(
            f"[OPSD Greedy Step {self.state.global_step}] Greedy generation done - "
            f"elapsed: {elapsed_time:.2f}s, prompts: {len(prompts_text)}, "
            f"tokens: {total_tokens}, speed: {total_tokens/max(elapsed_time, 1e-6):.1f} tok/s"
        )

        return completion_token_ids, completion_texts

    def _generate_continuation_vllm(self, prefix_token_ids_list: list[list[int]]) -> list[str]:
        ...
        if self.vllm_mode == "colocate":
            ...
            raise NotImplementedError(
                "Server mode continuation not yet implemented for OPSD greedy privileged. "
    ...
    def _build_teacher_prompts_from_answers(
        self, problems: list[str], answers: list[str]
    ) -> tuple[torch.Tensor, torch.Tensor, list[int], int]:
        """Build answer-conditioned teacher prompts dynamically from extracted greedy answers."""
        device = self.accelerator.device
        teacher_texts = []

        for problem, answer in zip(problems, answers):
            ref_solution = answer if answer else ""
            user_message = (
                f"Problem: {problem}\n\n"
    ...
    def training_step(
        ...
        # === GREEDY PRIVILEGED PHASE (if enabled) ===
        if self.use_greedy_privileged:
            # Build thinking-enabled prompts for greedy generation
            problems = inputs["problems"]
            greedy_prompts = []
            for problem in problems:
                user_message = f"Problem: {problem}\n\nPlease reason step by step, and put your final answer within \\boxed{{}}."
                messages = [{"role": "user", "content": user_message}]
                greedy_prompts.append(
                    self.processing_class.apply_chat_template(
                        messages, tokenize=False, add_generation_prompt=True, enable_thinking=True
                    )
                )

            # Greedy generation with thinking mode
            self._wake_vllm_if_needed()
            greedy_token_ids, greedy_texts = self._generate_greedy_privileged_vllm(greedy_prompts)

            # Extract answers
            greedy_answers = [extract_boxed_answer(t) for t in greedy_texts]

            # Prefix-guided continuation fallback for missing answers
            missing_indices = [i for i, a in enumerate(greedy_answers) if a in (None, "")]
            if missing_indices:
                prompt_encoded = self.processing_class(
                    greedy_prompts, padding=False, add_special_tokens=False
                )
                think_end_str = "</think>"
                boxed_prefix_str = "\n\n\\boxed{"
                ...
                for idx in missing_indices:
                    prompt_ids = prompt_encoded["input_ids"][idx]
                    comp_ids = greedy_token_ids[idx]
                    if think_end_str in greedy_texts[idx]:
                ...
                for i, idx in enumerate(missing_indices):
                    ...
                    if answer:
                        greedy_answers[idx] = answer

                forced_count = sum(
                    1 for idx in missing_indices if greedy_answers[idx] not in (None, "")
                )
                print(
                    f"[OPSD Greedy Step {self.state.global_step}] Prefix-guided continuation: "
                    f"{forced_count}/{len(missing_indices)} truncated completions recovered."
                )

            # Normalize None to empty string
            greedy_answers = [a if a else "" for a in greedy_answers]

            # Build teacher prompts from greedy answers
            problems = inputs["problems"]
            teacher_prompt_ids, teacher_prompt_attn, teacher_prompt_lengths, max_teacher_len = \
                self._build_teacher_prompts_from_answers(problems, greedy_answers)

            # Inject into inputs
            inputs["teacher_prompts"] = teacher_prompt_ids
            inputs["teacher_prompt_attention_mask"] = teacher_prompt_attn
            inputs["teacher_prompt_length"] = max_teacher_len
            inputs["teacher_prompt_lengths_per_example"] = torch.tensor(
                teacher_prompt_lengths, device=self.accelerator.device
            )

            # Metrics
            extractable = sum(1 for a in greedy_answers if a != "")
            g_extractable, g_answers = self._dist_reduce([float(extractable), float(len(greedy_answers))])
            if g_answers > 0:
                self._metrics["train"]["greedy/extraction_rate"].append(g_extractable / g_answers)

            if random.random() < 0.05:
                sample_idx = random.randint(0, len(greedy_answers) - 1)
                print(f"\n{'='*80}")
                print(f"GREEDY PRIVILEGED SAMPLE (Step {self.state.global_step}):")
                print(f"  Problem: {problems[sample_idx][:200]}...")
                print(f"  Greedy answer: {greedy_answers[sample_idx][:200]}")
                print(f"  Extraction rate: {extractable}/{len(greedy_answers)}")
                print(f"{'='*80}\n")
